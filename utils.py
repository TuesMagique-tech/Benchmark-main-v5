...
def setup_system(config):
    ...
    for key, value in config.items():
        if key.lower() == 'seed':
            seed_value = value
            break
    # If seed is not specified or set to 'random' or 'none', generate a random seed from current time
    if seed_value is None or (isinstance(seed_value, str) and seed_value.lower() in ['none', 'random']):
...
def Match2Pos_all(opt, region, config, uav_img0, finescale, K, ref_image, dsm_image, refLocY_list, refLocX_list, cut_H,
    ...
    and then uses the DSM data to solve a PnP problem to get the drone position.
    ...
    K(ndarray): the camera intrinsic matrix used to solve the PnP problem
    ref_image(ndarray): the 2D reference map used for AVL
    dsm_image(ndarray): the DSM map used for AVL
    refLocY_list(list) and refLocX_list(list): the positions of image retrieval results
    ...
    matRotation(matrix): the rotation matrix of the reference map (using prior yaw angle to align the drone image and reference map)'''
...
def pos2error(truePos, BLH_list, inliers_list):
    '''
    This function calculates the visual localization error e = sqrt((x_true - x_pred)^2 + (y_true - y_pred)^2).
    Note that we only care about the error of longitude and latitude, and do not consider the altitude error in this formula.
    The coordinate system is BLH (latitude-longitude-height). The top-n retrievals are re-ranked according to the number of PnP RANSAC inliers.
    truePos(dict): the ground truth of drone location
    BLH_list(list): the predicted locations (latitude, longitude, altitude) of the top-n images
    inliers_list(list): the inliers count of the top-n images
    '''
    ...
    for BLH_temp in BLH_list:
        if BLH_temp['L'] and BLH_temp['B']:
            ...
            location_error_list.append(10000)  # if there is no location data, set error to a large number
...
def resolution_size(data, opt):
    '''Estimate the spatial resolution of the drone image'''
...
def load_config_parameters_new(config, opt, region):
    ...
    # Read images
    # ref_image = cv2.imread(config[f'{region}_{Ref_type}_REF_PATH']) 换为如下Linux表达
    ref_path = config[f'{region}_{Ref_type}_REF_PATH']
    if '\\' in ref_path:
    ...
    # dsm_image = cv2.imread(config[f'{region}_{Ref_type}_DSM_PATH'], cv2.IMREAD_UNCHANGED).astype(np.float32)为如下Linux表达
    dsm_path = config[f'{region}_{Ref_type}_DSM_PATH']
    if '\\' in dsm_path:
        dsm_path = dsm_path.replace('\\', '/')
    dsm_image = cv2.imread(dsm_path, cv2.IMREAD_UNCHANGED).astype(np.float32)
...
def retrieval_init(method_dict, config):
    ...
    if method_dict['retrieval_method'] not in config['RETRIEVAL_METHODS']:
        ...
        method_dict['retrieval_model'], method_dict['img_transform'] = get_Model(method_dict['retrieval_method'], config=config)
...
def retrieval_all(ref_image, UAV_path, uav_data, ref_resolution, matRotation, save_path, opt, region, config, method_dict):
    ...
    uav_data(dict): the flight metadata including the ground truth of drone location and other imaging parameters
    ...
    initialX = config[f'{region}_{Ref_type}_REF_initialX']
    initialY = config[f'{region}_{Ref_type}_REF_initialY']
    # Estimate the geo-location of the center of the drone image
    ...
    # Filter out gallery images with too many invalid areas (black regions)
    ...
    # Extract the feature of drone image and reference tiles
    ...
    for (mid_x, mid_y) in mids:
        top = max(0, int(mid_x - block_size[0] / 2))
        left = max(0, int(mid_y - block_size[1] / 2))
        bottom = min(ref_image.shape[0], top + block_size[0])
        right = min(ref_image.shape[1], left + block_size[1])
        tile = ref_image[top:bottom, left:right]   # 裁剪参考切片图像
        ref_tiles.append(tile)
    # 分别为参考切片与 UAV 图像提取特征
    gf = extract_features(ref_tiles, method_dict, input_id=2)   # gallery features
    qf = extract_features(UAV_image, method_dict, input_id=1)   # query features


    retrieval_t1 = time.time()
    # Rank the gallery images (reference tiles) according to similarity
    ...
    # Calculate the retrieval error for PDM@K metric
    best_start_x = []
    best_start_y = []
    PDE_list = []
    ...
    if config['SHOW_RETRIEVAL_RESULT']:
        ...
        for i in range(min(len(mids), show_n)):
            ...
            if i == 0:
                ...
                fig.add_artist(
                    plt.Line2D([middle_position, middle_position], [0.25, 0.75], color='#CD2626', linestyle='dashed'))
            plt.imshow(img[..., ::-1])
...
def compute_block_mid_wo_black(image, block_size, step_size):
    ...
    # Resize the original image to improve efficiency
